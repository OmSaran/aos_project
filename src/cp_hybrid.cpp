...
bool sparse_copy(int src_fd, int dest_fd, char **abuf, size_t buf_size,
                 ...
                 uintmax_t max_n_read, off_t& total_n_read, ssize_t file_size)
{
    if (!*abuf)
    {
    ...
    while(max_n_read) {
        ssize_t n_read = MIN(max_n_read, buf_size);
        n_read = MIN(n_read, file_size);
        ...
        io_uring_prep_read(sqe, src_fd, *abuf, n_read, -1);
        ...
        io_uring_prep_write(sqe, dest_fd, ptr, n_read, -1);

        int ret = io_uring_submit(&ring);

        // while(io_uring_peek_cqe(&ring, &cqe) != 0); // use this for polling
        assert(io_uring_wait_cqe(&ring, &cqe) == 0);
        if(unlikely(cqe->res != n_read)) {
            if((int)cqe->res == 0 && (int)cqe->user_data == 2) {
                std::cerr << "Read 0 bytes for file " << src_name << std::endl;
                exit(1);
                break;
            } else {
                std::cerr << "Failed to write: " << strerror(-cqe->res) << "; User data = " << (int)cqe->user_data << std::endl;
                assert(0);
                exit(1);
            }
        }
              ...
              const struct stat& src_sb)
{
    ...
    sparse_copy(source_desc, dest_desc, &buf, buf_size,
                src_name, dst_name, UINTMAX_MAX, n_read, src_open_sb.st_size);
...
int main(int argc, char** argv)
{
    ...
    ring_ret = io_uring_queue_init_params(8, &ring, &params);
    if(ring_ret != 0) {
        std::cerr << "Failed to setup iouring: " << strerror(-ring_ret) << std::endl;
        exit(1);
    }
