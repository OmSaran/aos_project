...
FILE_SIZES = [2**20, 2**21, 2**22, 2**23]
...
class MultiFileFileSizeGraph(BaseFileGraph):
    NAME = 'graph_bench_multi_file_size'
    def __init__(self, target_dir, bin_dir, skip_sanity, skip_drop, num_files, sq_poll):
    ...
    def create_required_workloads(self):
        for file_size in self._file_sizes:
            root_path = self.get_root_path(file_size)
            self._created_directories.append(root_path)
            work_gen = DirCreator(root_path)
            work_gen.create(1, 0, self._num_files, (file_size, file_size))
